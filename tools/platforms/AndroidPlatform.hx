...
class AndroidPlatform extends PlatformTarget
{
	...
	public override function build():Void
	{
		...
		{
			var haxeParams = [hxml, "-D", "android", "-D", "PLATFORM_NUMBER=21"];
			var cppParams = ["-Dandroid", "-DPLATFORM_NUMBER=21"];
	...
	public override function rebuild():Void
	{
		...
		if (armv5) commands.push(["-Dandroid", "-DPLATFORM_NUMBER=21"]);
		if (armv7) commands.push(["-Dandroid", "-DHXCPP_ARMV7", "-DPLATFORM_NUMBER=21"]);
		if (arm64) commands.push(["-Dandroid", "-DHXCPP_ARM64", "-DPLATFORM_NUMBER=21"]);
		if (x86) commands.push(["-Dandroid", "-DHXCPP_X86", "-DPLATFORM_NUMBER=21"]);
		if (x64) commands.push(["-Dandroid", "-DHXCPP_X86_64", "-DPLATFORM_NUMBER=21"]);
	...
	public override function update():Void
	{
		...
		context.ANDROID_TARGET_SDK_VERSION = project.config.getInt("android.target-sdk-version", 33);
